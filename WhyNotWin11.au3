...
Func CopyLangFiles()
	Local $fileList, $iFile
	; Searching only FILES matches *.lang into langs folder
	$fileList = _FileListToArray(".\langs\", "*.lang", $FLTA_FILES)
	; Starting from 2 to ignore 0000.lang
	For $iFile = 2 To $fileList[0]
		FileInstall(".\langs\" & $fileList[$iFile], @LocalAppDataDir & "\WhyNotWin11\Langs\" & $fileList[$iFile], $FC_OVERWRITE)
	Next
